...
try{
 ...
 await page.goto('http://localhost:5173/');assert.equal(await page.title(),'topo.ski');assert.equal(await page.locator('.mountain-tile').count(),61);
 ...
 await page.locator('#settings-toggle').click();await page.locator('#atlas-tab').click();await page.locator('[data-pass="epic"]').click();assert.equal(new URL(page.url()).pathname,'/palisades');assert.equal(await page.locator('#mountain-heading').textContent(),'Palisades Tahoe');
 await page.keyboard.press('Escape');assert.equal(await page.locator('#field-guide').isHidden(),true);assert.equal(await page.locator('#settings-toggle').evaluate(e=>e===document.activeElement),true);
 await page.locator('#settings-toggle').click();await page.locator('#weather-drawer').click();assert(await page.locator('#weather-timeline').isVisible());await page.keyboard.press('Escape');assert(await page.locator('#weather-timeline').isHidden());
 await page.locator('#settings-toggle').click();await page.locator('#motion-toggle').click();const paused=await page.evaluate(()=>window.getAtlas().elapsed);await page.waitForTimeout(200);assert.equal(await page.evaluate(()=>window.getAtlas().elapsed),paused);
 ...
 await page.locator('#settings-toggle').click();await page.locator('[data-layer="lifts"]').click();assert.equal(await page.evaluate(()=>window.getAtlas().liftGroup.visible),false);await page.locator('[data-layer="lifts"]').click();
 await page.locator('#close-settings').click();await page.locator('#close-route').click();await page.waitForTimeout(1000);
 ...
 const mobile=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});mobile.on('pageerror',e=>errors.push(e.message));await mobile.goto('http://localhost:5173/#palisades');await mobile.waitForFunction(()=>document.querySelector('#scene[aria-busy="false"]'));await mobile.screenshot({path:'/tmp/schema-mobile.png'});assert.equal(await mobile.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);assert(await mobile.locator('#pan-mode').isHidden());await mobile.locator('#settings-toggle').click();assert(await mobile.locator('#pan-mode').isVisible());
