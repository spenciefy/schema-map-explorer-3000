...
export class AtlasScene {
 ...
 suspended=false;paused=false;reducedMotion=matchMedia('(prefers-reduced-motion: reduce)').matches;showSnow=true;showTrails=true;showLifts=true;showLabels=false;showRoads=true;showPlaces=true;contextGroup=new THREE.Group();buildingGroup=new THREE.Group();
 ...
 constructor(public host:HTMLElement,public labelHost:HTMLElement,public onSelect:(id:string)=>void){
  ...
  this.renderer.domElement.setAttribute('role','img');this.renderer.domElement.setAttribute('aria-label','Geographic 3D trail map. Drag or use arrow keys to pan, right-drag to rotate, scroll to zoom. Search and select named trails and lifts in the mountain explorer.');this.renderer.domElement.tabIndex=0;host.append(this.renderer.domElement);
  ...
  this.controls.addEventListener('start',()=>{this.targetCamera=null;this.targetLook=null;this.journey=undefined;this.host.style.cursor='grabbing';});this.controls.addEventListener('end',()=>this.host.style.cursor='grab');
 ...
 resize(){const w=this.host.clientWidth,h=this.host.clientHeight;this.renderer.setSize(w,h);this.camera.aspect=w/h;this.camera.updateProjectionMatrix();const m=innerWidth<760;if(m!==this.mobile){this.mobile=m;this.home(true);}}
 ...
 async load(resort:Resort){const request=++this.requestId;this.host.classList.add('terrain-loading');this.labelHost.classList.add('terrain-loading');this.data=undefined;this.features=[];this.world.visible=false;this.labels.forEach(l=>l.el.hidden=true);this.tooltip.hidden=true;this.journey=undefined;this.traveler.visible=false;
  ...
  try{let data=cache.get(resort.id);if(!data){const response=await fetch(`/geodata/${resort.id}.json`);if(!response.ok)throw new Error('Mountain data unavailable');data=await response.json() as MountainData;if(data.gridSize**2!==data.heights.length)throw new Error('Invalid terrain grid');cache.set(resort.id,data);}if(request!==this.requestId)return;
   this.clear();this.data=data;this.scale=240/Math.max(data.width,data.depth);this.minElevation=Math.min(...data.heights);this.maxElevation=Math.max(...data.heights);
   this.features=data.features.flatMap(f=>clipFeature(data!,f));
   this.coverImage=undefined;if(data.landcover){try{const cover=new Image();await new Promise<void>((resolve,reject)=>{cover.onload=()=>resolve();cover.onerror=reject;cover.src=data!.landcover!;});if(request!==this.requestId)return;this.coverImage=cover;}catch{console.warn('Land cover unavailable; using mapped forest polygons.');}}
 ...
 clearFocus(){this.selectedFeature=undefined;if(this.highlight){this.world.remove(this.highlight);this.highlight.geometry.dispose();(this.highlight.material as THREE.Material).dispose();this.highlight=undefined;}this.labels.forEach(l=>l.el.classList.remove('chosen'));}
 home(immediate=false){if(!this.data)return;this.clearFocus();this.journey=undefined;this.traveler.visible=false;const d=this.data,height=(this.maxElevation-this.minElevation)*this.scale*this.exaggeration;const liftPoints=this.features.filter(f=>f.kind==='lift'&&!f.proposed).flatMap(f=>f.points).filter(p=>inExtent(d,p));const xs=liftPoints.map(p=>p[0]*this.scale),zs=liftPoints.map(p=>p[1]*this.scale);const mx=xs.length?(Math.min(...xs)+Math.max(...xs))/2:0,mz=zs.length?(Math.min(...zs)+Math.max(...zs))/2:0;const look=new THREE.Vector3(mx,this.height(mx,mz)*.35,mz);const ref=mapReferences[this.selected];const v=new THREE.Vector3(ref?.view[0]||0,0,ref?.view[1]||1).normalize();const distance=this.mobile?430:325;const pos=this.topDown?look.clone().add(new THREE.Vector3(0,distance,.1)):look.clone().add(v.multiplyScalar(distance*.84)).add(new THREE.Vector3(0,distance*.61,0));
 ...
 zoom(factor:number){this.journey=undefined;const offset=this.camera.position.clone().sub(this.controls.target).multiplyScalar(factor);offset.clampLength(15,600);this.targetCamera=this.controls.target.clone().add(offset);this.targetLook=this.controls.target.clone();}
 ...
 animate=()=>{this.animationId=requestAnimationFrame(this.animate);if(document.hidden||this.suspended){this.clock.getDelta();return;}const delta=Math.min(this.clock.getDelta(),.05),moving=!this.paused&&!this.reducedMotion;if(moving)this.elapsed+=delta;const time=this.elapsed;
  ...
  if(this.skiers&&this.boarders){this.actors.forEach(actor=>{
   ...
   placeRider(this.dummy,p,v,(x,z)=>this.height(x,z),carve);(actor.board?this.boarders!:this.skiers!).setMatrixAt(actor.index,this.dummy.matrix);
