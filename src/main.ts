...
app.innerHTML=`
  <section id="mountain-gallery" aria-label="Mountains"><div class="gallery-heading"><h1>Mountains</h1><span>${resorts.length}</span></div><div class="mountain-grid">${resorts.map(r=>`<a class="mountain-tile" href="#${r.id}" aria-label="Explore ${r.name}"><div class="mountain-preview"><img src="/previews/${r.id}.png" alt="" width="720" height="520" loading="${resorts.indexOf(r)<6?'eager':'lazy'}" /></div><div class="tile-caption"><div><h2>${r.name}</h2><span>${r.area} · ${r.country}</span></div><span class="tile-arrow" aria-hidden="true">↗</span></div></a>`).join('')}</div><div class="gallery-footer"><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">© OpenStreetMap</a><a href="https://registry.opendata.aws/terrain-tiles/" target="_blank" rel="noopener">Mapzen terrain</a><a href="https://doi.org/10.5281/zenodo.7254221" target="_blank" rel="noopener">ESA WorldCover</a></div></section>
  ...
  <main>
    ...
    <aside class="mountain-panel" id="field-guide" aria-label="Mountain field guide" hidden><button id="close-guide" aria-label="Close mountain field guide">×</button>
      <div class="panel-topline"><span class="eyebrow">Explore</span><span id="resort-index">05 / 06</span></div>
    ...
    <div class="terrain-controls"><div class="interaction-modes" role="group" aria-label="Drag mode"><button id="pan-mode" class="active" aria-pressed="true" title="Drag to move across the mountain">Pan</button><button id="rotate-mode" aria-pressed="false" title="Drag to orbit the mountain">Rotate</button></div><div class="view-presets" role="group" aria-label="Map perspective"><button id="panorama-view" class="active" aria-pressed="true">Trail-map view</button><button id="overhead-view" aria-pressed="false">Overhead</button></div><div class="layer-toggles" role="group" aria-label="Map layers"><button data-layer="trails" aria-pressed="true"><span class="layer-check">✓</span> Trails</button><button data-layer="lifts" aria-pressed="true"><span class="layer-check">✓</span> Lifts</button><button data-layer="roads" aria-pressed="true"><span class="layer-check">✓</span> Roads</button><button data-layer="places" aria-pressed="true"><span class="layer-check">✓</span> Lodges</button><button data-layer="labels" aria-pressed="false"><span class="layer-check">−</span> Names</button></div><a id="official-map" target="_blank" rel="noopener">Official trail map ↗</a><a id="google-map" target="_blank" rel="noopener">Google Maps ↗</a></div>
    <div class="terrain-status" id="terrain-status" role="status">Loading real mountain terrain…</div>
...
function renderDetail(){
 ...
 $('#official-map').setAttribute('href',mapReferences[r.id].url);$('#official-map').textContent=(mapReferences[r.id].label||'Official trail map')+' ↗';$('#google-map').setAttribute('href',`https://www.google.com/maps/search/?api=1&query=${r.lat},${r.lon}`);$('#download-geodata').setAttribute('href',`/geodata/${r.id}.json`);
 ...
 $('#save-mountain').onclick=()=>{if(saved.has(r.id))saved.delete(r.id);else saved.add(r.id);try{localStorage.setItem('chasing-winter:saved',JSON.stringify([...saved]));}catch{}render();notify(saved.has(r.id)?`${r.name} saved.`:`${r.name} removed.`);};
 $('#explore-mountain').onclick=()=>{explorer='trails';featureQuery='';$<HTMLInputElement>('#feature-search').value='';renderList();notify('Select a trail.');};
...
function renderList(){
 ...
 const list=visibleResorts();$('#visible-count').textContent=`${list.length} PLACES`;$('#list-title').textContent=view==='saved'?'Saved':'Mountains';
...
$('#weather-switch').onclick=()=>{live=!live;hour=live?0:17;weatherError=false;weatherLoading=false;$('#weather-switch').setAttribute('aria-pressed',String(live));$('#mode-name').textContent=live?'Live forecast':'Demo weather';$('#weather-switch').classList.toggle('is-live',live);if(live)fetchWeather();else{requestId++;render();}notify(live?'Current modeled weather at mountain elevation.':'Simulated weather.');};
...
function toggleGuide(open:boolean){$('#field-guide').hidden=!open;document.body.classList.toggle('guide-open',open);$('#atlas-tab').setAttribute('aria-expanded',String(open));if(open)$('#close-guide').focus();}
...
$('#close-guide').onclick=()=>{toggleGuide(false);$('#atlas-tab').focus();};
...
window.addEventListener('keydown',e=>{if(e.key==='Escape'){toggleGuide(false);$('#weather-timeline').hidden=true;$('#weather-drawer').setAttribute('aria-expanded','false');$('#weather-drawer span').textContent='＋';}});
...
function renderFeatureList(){
 if(explorer==='stops'){const items=(terrainData?.places||[]).filter(p=>inExtent(terrainData!,p.point)&&p.name.toLocaleLowerCase().includes(featureQuery.toLocaleLowerCase()));$('#list-title').textContent='Lodges';$('#visible-count').textContent=`${items.length} MAPPED`;$('#resort-list').innerHTML=items.length?items.map((p,i)=>`<button class="feature-row" data-place-index="${i}"><span class="difficulty-symbol lodge">⌂</span><span>${escapeHtml(p.name)}<small>${escapeHtml(p.kind)} · mapped location</small></span>${icon('arrow')}</button>`).join(''):`<div class="empty-state">${terrainData?'No mapped stops match this search.':'Loading mountain places…'}</div>`;document.querySelectorAll<HTMLButtonElement>('[data-place-index]').forEach(el=>el.onclick=()=>atlas?.focusPlace(items[Number(el.dataset.placeIndex)]));return;}
...
window.addEventListener('terrain-loading',()=>{terrainData=undefined;mapFeatures=[];activeFeature=undefined;$('#route-card').hidden=true;$('#terrain-status').textContent='Loading map…';$('#terrain-status').classList.remove('error');if(explorer!=='places')renderList();});
window.addEventListener('terrain-ready',(event)=>{const detail=(event as CustomEvent).detail;terrainData=detail.data;mapFeatures=detail.features;const liftNames=new Set(mapFeatures.filter(f=>f.kind==='lift'&&f.name).map(f=>f.name));const trailNames=new Set(mapFeatures.filter(f=>f.kind==='trail'&&f.name).map(f=>f.name));$('#terrain-status').textContent=`${trailNames.size} named runs · ${liftNames.size} mapped lifts · Real elevation`;$('#terrain-status').title=`${mapReferences[selected].coverage}. OSM snapshot ${terrainData!.source.osmTimestamp}. DEM grid spacing ${terrainData!.source.sampleSpacingMeters.join(' × ')} m; native resolution varies.`;if(explorer!=='places')renderList();});
window.addEventListener('terrain-error',()=>{$('#terrain-status').textContent='Terrain unavailable. Choose another mountain or reload.';$('#terrain-status').classList.add('error');});
...
for(const mode of ['pan','rotate'] as const)$('#'+mode+'-mode').onclick=()=>{atlas?.setInteractionMode(mode);for(const value of ['pan','rotate']){const button=$('#'+value+'-mode');button.classList.toggle('active',value===mode);button.setAttribute('aria-pressed',String(value===mode));}};
