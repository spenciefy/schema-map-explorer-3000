...
let galleryScroll=0;
function navigate(){let id='';try{id=decodeURIComponent(location.hash.slice(1));}catch{}const resort=resorts.find(r=>r.id===id);const gallery=!resort;if(resort&&document.body.classList.contains('gallery-view'))galleryScroll=window.scrollY;document.body.classList.toggle('gallery-view',gallery);$('#mountain-gallery').hidden=!gallery;
...
function toggleGuide(open:boolean){if(open&&$('#field-guide').hidden)guideTrigger=document.activeElement as HTMLElement;$('#field-guide').hidden=!open;document.body.classList.toggle('guide-open',open);$('#atlas-tab').setAttribute('aria-expanded',String(open));$('#saved-tab').setAttribute('aria-expanded',String(open&&view==='saved'));if(open)$('#close-guide').focus();}
...
window.addEventListener('keydown',e=>{if(e.key==='Escape'){if(!$('#field-guide').hidden){toggleGuide(false);guideTrigger?.focus();}else if(!$('#route-card').hidden){$('#route-card').hidden=true;atlas?.clearFocus();atlas?.renderer.domElement.focus();}else if(!$('#weather-timeline').hidden)$('#weather-drawer').focus();$('#weather-timeline').hidden=true;$('#weather-drawer').setAttribute('aria-expanded','false');$('#weather-drawer span').textContent='＋';}});
