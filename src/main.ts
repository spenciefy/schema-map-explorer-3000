...
app.innerHTML=`
  ...
  <main>
    ...
    <aside id="rider-settings" class="rider-settings" aria-labelledby="settings-title" hidden>
      <div class="settings-heading"><h2 id="settings-title">On the mountain</h2><button id="close-settings" aria-label="Close settings">${icon('close')}</button></div>
      ${(['skier','snowboarder'] as const).map(kind=>`<fieldset><legend>${kind==='skier'?'Skiers':'Snowboarders'}</legend><label class="settings-visibility"><span>Show ${kind}s</span><input id="${kind}-visible" type="checkbox" data-rider-setting="${kind==='skier'?'skiers':'snowboarders'}" /></label><label class="settings-range" for="${kind}-size"><span>Size <output id="${kind}-size-value" for="${kind}-size"></output></span><input id="${kind}-size" data-rider-setting="${kind}Size" type="range" min="0.5" max="2" step="0.05" /></label><label class="settings-range" for="${kind}-speed"><span>Speed <output id="${kind}-speed-value" for="${kind}-speed"></output></span><input id="${kind}-speed" data-rider-setting="${kind}Speed" type="range" min="0" max="3" step="0.1" /></label></fieldset>`).join('')}
      <p id="rider-settings-note">Size and speed are relative to the default.</p><button id="reset-rider-settings">Reset defaults</button>
    </aside>
  </main>
...
function ensureAtlas(){if(atlas)return;try{atlas=new AtlasScene($('#scene'),$('#map-labels'),id=>selectResort(id,true));atlas.riderSettings={...riderSettings};}catch{$('#scene').innerHTML='<div class="canvas-error">3D map unavailable. Try a browser with WebGL enabled.</div>';}}
let galleryScroll=0;
...
function renderRiderSettings(){
 ...
 $('#rider-settings-note').textContent=skiOnly?'This mountain is skiers only. Snowboarder preferences still apply elsewhere.':'Size and speed are relative to the default. Set speed to zero to hold riders still.';
}
...
$('#reset-rider-settings').onclick=()=>{riderSettings={...defaultRiderSettings};applyRiderSettings();notify('Rider settings reset.');};
navigate();
