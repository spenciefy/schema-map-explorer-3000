...
export class MountainGlobe {
 ...
 constructor(private host:HTMLElement,private select:(resort:Resort)=>void){
  this.renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});this.renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));host.append(this.renderer.domElement);this.renderer.domElement.setAttribute('aria-label','Mountain globe. Drag to rotate and scroll to zoom. Use the mountain list to select with a keyboard.');this.renderer.domElement.setAttribute('role','img');
